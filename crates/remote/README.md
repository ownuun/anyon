...
The `remote` crate contains the implementation of the Anyon hosted API.
...
```env
ANYON_REMOTE_JWT_SECRET=your_base64_encoded_secret
...
Generate `ANYON_REMOTE_JWT_SECRET` once using `openssl rand -base64 48` and copy the value into `.env.remote`.
...
```bash
export ANYON_SHARED_API_BASE=http://localhost:8081
